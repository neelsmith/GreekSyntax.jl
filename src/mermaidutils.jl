function mermaiddiagram(sa::SentenceAnnotation, tknannotations::Vector{TokenAnnotation})
    ...
    for t in lextokens
        ...
        if isnothing(t.node1relation) || isnothing(t.node1)
            ...
            push!(graphlines, string(tknidx, "[", t.text, "]", " --> |", t.node1relation, "| ", t.node1, "[", lextokens[n1int],  "];"))
			...
			if  isnothing(t.node2) || isempty(t.node2)
                ...
                push!(graphlines, string(tknidx, "[", t.text, "]", " --> |", t.node2relation, "| ", t.node1, "[", lextokens[n2int],  "];"))
			end
        end
    end
